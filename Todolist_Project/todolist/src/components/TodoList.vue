        ...
        <el-row :key="item.id">
          ...
          <el-col :span="8">
            <el-col :span="1">
              <el-checkbox
                v-model="item.value"
                @change="updateItem(item.value,item.id,item.content)"
              ></el-checkbox>
            </el-col>
            <el-col :span="10" class="ellipsis">{{item.content}}</el-col>
            <el-col :span="5">
              <div>
                <el-link icon="el-icon-edit" @click="triggerChange(true,item)"></el-link>
              </div>
            </el-col>
            <el-col
              :span="6"
...
export default {
  ...
  methods: {
    ...
    changeStatus (sitem) {
      ...
      let title = sitem.title
      this.updateItem(value, id, title)
    },
    updateItem (value, id, title) {
